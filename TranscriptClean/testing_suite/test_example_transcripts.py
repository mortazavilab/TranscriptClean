...
class TestCorrectTranscripts(object):
    def test_DIM_nc(self):
        ...
        tmp_dir = f"{test_dir}/scratch/example/TC_tmp/"
    ...
    def test_DIM_nc_full(self):
        ...
        #            "--canonOnly", "--o", f"{test_dir}/scratch/DIM_nc_full/TC"]
        ...
        # Read in transcript from outfile
        with open(f"{test_dir}/scratch/DIM_nc_full/TC_clean.sam", 'r') as f:
            sam_line = f.readline().strip().split('\t')
        ...
        # Test the sequence
        with open(f"{test_dir}/answer_files/DIM_nc_full/TC_clean.sam", 'r') as f:
            sam_line = f.readline().strip().split('\t')
        ...
        with open(f"{test_dir}/scratch/DIM_nc_full/TC_clean.log", 'r') as f:
        ...
        counter = 0
        with open(f"{test_dir}/scratch/DIM_nc_full/TC_clean.TE.log", 'r') as f:
            for line in f:
                print(line)
        ...
        #            f"{test_dir}/scratch/DIM_nc_full/TC"]
        #
        # try:
        #     output = subprocess.check_output(command)
        #
        ...
        # assert os.path.getsize(f"{test_dir}/scratch/DIM_nc_full/TC_report.pdf") > 0
